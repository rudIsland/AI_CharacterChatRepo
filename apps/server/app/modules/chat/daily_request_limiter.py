...
class DailyRequestLimiter:
    def __init__(self, daily_request_limit: int, daily_request_limit_per_ip: int) -> None:
    ...
    def increase_request_count(self, ip_address: str) -> DailyRequestLimitResult:
        with self._write_lock:
            ...
            current_ip_request_count = self._daily_request_count_by_ip.get(ip_address, 0)
            if (
                self._daily_request_limit_per_ip > 0
                and current_ip_request_count >= self._daily_request_limit_per_ip
            ):
                return DailyRequestLimitResult(is_allowed=False, error_message="오늘 이 IP에서 사용할 수 있는 AI 요청 수를 모두 사용했습니다.")

            if (
                self._daily_request_limit > 0
                and self._daily_request_count >= self._daily_request_limit
            ):
                return DailyRequestLimitResult(is_allowed=False, error_message="오늘 전체 AI 요청 수를 모두 사용했습니다.")
    ...
    def set_ip_request_count(self, ip_address: str, request_count: int) -> None:
        ...
        with self._write_lock:
            ...
            current_ip_request_count = self._daily_request_count_by_ip.get(ip_address, 0)
            self._daily_request_count = max(0, self._daily_request_count - current_ip_request_count)
    ...
    def reset_ip_usage(self, ip_address: str) -> None:
        with self._write_lock:
            ...
            current_ip_request_count = self._daily_request_count_by_ip.pop(ip_address, 0)
            self._daily_request_count = max(0, self._daily_request_count - current_ip_request_count)
