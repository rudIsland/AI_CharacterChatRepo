...
function getRequiredApiBaseUrl(): string {
  ...
  if (!apiBaseUrl) {
    throw new Error("NEXT_PUBLIC_API_BASE_URL is required. Set it in apps/client/web/.env.");
  }
  return apiBaseUrl;
}
...
export async function updateAdminDailyRequestIpCount(adminApiKey: string, ipAddress: string, dailyRequestCount: number): Promise<AdminDailyRequestUsageResponse> {
  return chatApiClient.updateAdminDailyRequestIpCount(adminApiKey, ipAddress, dailyRequestCount);
}
