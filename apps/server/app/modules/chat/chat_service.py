...
class ChatService:
    ...
    def list_chat_message_by_session_id(self, chat_session_id: str) -> ChatSessionMessageListResponse:
    ...
    async def create_chat_message(
        ...
        self._raise_if_ai_model_unavailable(selected_ai_model_id)
        self._chat_store.update_chat_session_ai_model_id(chat_session_id=chat_session_id, ai_model_id=selected_ai_model_id)

        user_message = self._chat_store.append_chat_message(chat_session_id=chat_session_id, role="user", message_text=user_message_text)

        fixed_reply_text = find_character_easter_egg_reply(character_id=character.character_id, user_message_text=user_message_text)
        if fixed_reply_text is not None:
            assistant_message = self._chat_store.append_chat_message(chat_session_id=chat_session_id, role="assistant", message_text=fixed_reply_text)
        ...
        current_message_list = self._chat_store.list_chat_message_by_session_id(chat_session_id)
        history_turn_list = [
            ChatHistoryTurn(role=message.role, message_text=message.message_text)
        ...
        assistant_reply_result = await self._ai_reply_service.generate_character_reply(character_prompt_profile=character_prompt_profile, chat_history=history_turn_list, ai_model_id=selected_ai_model_id)
        ...
        assistant_message = self._chat_store.append_chat_message(
            ...
            input_token_count=token_usage.input_token_count if token_usage is not None else None,
            output_token_count=token_usage.output_token_count if token_usage is not None else None,
            total_token_count=token_usage.total_token_count if token_usage is not None else None,
        )
    ...
    def _build_chat_session_summary(self, chat_session: StoredChatSession) -> ChatSessionSummary:
        return ChatSessionSummary(
            chat_session_id=chat_session.chat_session_id,
            character_id=chat_session.character_id,
            guest_id=chat_session.guest_id,
            ai_model_id=chat_session.ai_model_id,
            created_at=chat_session.created_at,
        )

    def _build_chat_message(self, chat_message: StoredChatMessage) -> ChatMessage:
        return ChatMessage(
    ...
    def _calculate_used_token_count(self, message_list: list[StoredChatMessage]) -> int:
        used_token_count = 0
    ...
    def _raise_if_ai_model_unavailable(self, ai_model_id: AiModelId) -> None:
        if is_ai_model_available(ai_model_id=ai_model_id, app_settings=self._app_settings):
            return

        raise AiModelUnavailableError(get_unavailable_ai_model_message(ai_model_id))
